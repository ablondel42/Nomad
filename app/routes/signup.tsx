...
export function meta() {
  return [
    { title: "Sign Up - Nomad Jobs" },
    { name: "description", content: "Join Nomad Jobs today." },
  ]
}
...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Auto redirect to home, Supabase usually logs them in immediately if email confirmation is off.
      navigate('/')
    }
  }
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-slate-50 dark:bg-slate-950 px-4 py-20 flex justify-center items-center">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-10 lg:p-12 border-2 border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-8 text-center">Create an Account</h2>
        ...
        <form onSubmit={handleSignup} className="space-y-8">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full mt-4 bg-blue-600 text-white px-8 py-3.5 rounded-xl font-bold hover:bg-blue-700 border-2 border-transparent transition-all active:scale-[0.98] disabled:opacity-50"
          >
